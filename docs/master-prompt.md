...
> - カテゴリをオーナーの実際の利用に合わせて調整（**ニットはトップスへ統合 / つなぎ系はトップス**）
> - **袖丈**を属性として追加（半袖 / 長袖 / ノースリーブ / 七分袖）。トップス・シャツでは一覧から直接切り替えられる
> - 経緯と判断は [decisions.md](decisions.md) の ADR-005・ADR-007 と [status.md](status.md) を参照
...
2026-09-13 改訂:
背景除去（端末内ONNX）は削除した。
精度が実用に足りず、releaseビルドでJNIまわりのクラッシュを起こし、
約33MBのネイティブライブラリを同梱していたため（ADR-006）。
正規化（向き・切り出し・中央配置・余白・thumbnail）のみ行い、
原本は保持して詳細画面で切り替えられるようにする。
以下は当初の要件として残す。
...
2026-09-14 改訂:
オーナーの実際の利用に合わせて、
デニム・セットアップ・バッグを削除し、スーツを追加した。
ニットは独立カテゴリにせずトップスへ統合する。
オールインワン・つなぎ・ジャンプスーツ・カバーオールも
トップスに入れ、細分類はsubCategoryに保持する。
ワンピース・スカートは追加しない。
理由と経緯は docs/decisions.md の ADR-005・ADR-007 を参照。
...
一覧部分を左右にスワイプしても、隣のカテゴリへ移動できる。
一覧は指のドラッグ量へ追従して横移動し、離すとページ単位で滑らかにスナップする。
スワイプ時は選択中のカテゴリ名を見える位置へ自動で移動する。

トップスまたはシャツを選んだときは、その直下に
「すべて / 半袖 / 長袖 / ノースリーブ / 七分袖」の
袖丈ショートカットを表示する。別カテゴリへ移ると袖丈条件を解除する。
...
0004_merge_knitwear.sql  ニットのトップス統合・つなぎ系の再分類
...
- ニットカテゴリを削除し、既存のニットをトップスへ移行
- オールインワン・つなぎ・ジャンプスーツ・カバーオールをトップスとして扱う
- トップスとシャツの一覧に袖丈ショートカット（すべて / 半袖 / 長袖 / ノースリーブ / 七分袖）を追加
- 一覧の左右スワイプでもカテゴリを切り替えられるようにし、下部バーを廃止。服の追加は右下の黒い円形ボタン（白い＋）に整理
- カテゴリ切替をPageView化し、指へ追従する横移動とスナップアニメーションに変更
- アイコンのRobotoカーニングと字間をクローゼット見出しへ揃え、背景をアプリ共通の `#FAFAF8` に変更

2026-09-13

- アプリの表示名を WDRB に変更（リポジトリ名等は wadrob のまま）
- アイコンとスプラッシュをアプリ背景・黒文字のワードマークで整備
  （書体と字間はクローゼット見出しと同じ Roboto。tool/generate_brand_assets.py で再生成）
- カテゴリを実際に着る物へ調整
  アウター / トップス / シャツ / パンツ / スーツ / シューズ / アクセサリー / その他
