...
- `GET /api/items?status=active&category=&brand=&normalizedColor=&size=&search=&sort=recent` → `{ items }`。
- `GET /api/items/:id` → `{ item }`。
- `POST /api/items` / `PUT /api/items/:id` — 商品fieldと `imageIds` / `imageUrls`。`name`のみ必須。金額は0以上の整数。通貨は3文字。URLはHTTP/HTTPS。
...
- `POST /api/classify` — `{ "name": "タートルネック ニット セーター", "brand": "classicalelf" }` → `{ category?, normalizedColor?, subCategory? }`。名前からカテゴリ・検索用カラーを推定する。許可値のenumで拘束し、判断できない項目は返さない。保存しない。
