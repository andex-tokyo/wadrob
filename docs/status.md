...
- クローゼット: 2/3/4列グリッド（密度保存）、カテゴリはタップと一覧の左右スワイプで切替、インライン検索、複合フィルタ、ソート、Pull to Refresh、空状態・エラー時の非破壊表示。下部は「服を追加」だけの主操作に整理
...
- 2026-09-14: 一覧の左右スワイプで前後のカテゴリへ移動し、選択タブも追従する操作を追加。無意味だった下部「クローゼット」を削除し、黒い「服を追加」ボタン1つに整理
