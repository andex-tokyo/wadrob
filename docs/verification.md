...
- 下部バーを削除して一覧の表示領域を拡大し、右下に黒い円形の「＋」追加ボタンを配置。widget testで旧「クローゼット」が無いこと、FABの前景色・背景色を確認
...
- Worker Vitest 45 tests（商品ID優先・最頻グループの絞り込み・クエリ違いの統合・上限を追加）

追記（Yahooの関連商品対策）: Yahoo!ショッピングでは関連商品の画像が混ざることがあった。トークン判定を「数字を含む5文字以上の英数字」に広げ、`yctops82` のような型番にも対応した。実測で ZOZO店の2ページは 28枚 / 36枚（うち34枚が同一商品、残りはYahoo自身が指定するOGP画像）で、`lingxiayuu` は25/26枚が同一商品になった。

追記（UNIQLOの色欠け）: 「ほしい色の写真が出ない」事象は2つの原因があった。1つは `/AsianCommon/` を `/common/` のUI除外ルールで消していたこと（パス区切りを意識する形に修正）。もう1つは、私が入れていた「ファイル名の先頭で最頻グループだけ残す」判定で、UNIQLOの `jpgoods_*`（日本向け15色）と `goods_*`（アジア共通15色）の片方が落ちていたこと（トークン判定で足りるため削除）。修正後は **30色・38枚すべての色が候補に入る**。
