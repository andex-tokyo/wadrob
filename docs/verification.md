...
追記（Yahooの関連商品対策）: Yahoo!ショッピングでは関連商品の画像が混ざることがあった。トークン判定を「数字を含む5文字以上の英数字」に広げ（`yctops82` のような型番にも対応）、さらに**ファイル名の先頭でグループ化して最頻グループ（5枚以上）だけを残す**判定を追加。実測で ZOZO店の2ページはいずれも同一商品のみ（28枚 / 34枚）、`lingxiayuu` の商品は25枚すべてが同一商品（`yctops82*`）になり、UUID名の混入画像も消えた。
