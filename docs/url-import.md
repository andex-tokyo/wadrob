...
候補は最大40枚。次の2段階で関連商品やUIアセットを落とす。

1. 商品IDなどのトークン（URLや既知の画像に含まれる、数字を含む5文字以上の英数字。`106137321b` や `yctops82` のような型番も拾う）と一致する画像を優先し、一致が5枚以上なら他を落とす
2. ファイル名の先頭（`_` や `.` の前、末尾の `-01` 等は無視）でグループ化し、**最頻グループが5枚以上ならそのグループだけを残す**

同じ商品のギャラリーはファイル名の先頭が揃う（`106137321b_*`、`yctops82_*`、`4021570436_*`）ため、2の判定で関連商品が消える。UNIQLOのように関連商品が181枚並ぶページでも、その商品の画像だけが残る。
